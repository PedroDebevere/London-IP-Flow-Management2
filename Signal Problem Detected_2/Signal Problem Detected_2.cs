...
public class Script
{
	...
	{
		string message = "An error was detected with the uplink in {0}.\r\n\r\nCheck live signal feed from satellite downlink.\r\n\r\nDo you want to switch the uplink to the alternate site in {1}?";
		string setParam = "";
		int setVal = 0;
		UIResults uir = null;
		string btnValue = "";
				...
				if ((trigger[2] == "SITE1" && Convert.ToString(Switch.GetParameter(300)) == "1") || (trigger[2] == "SITE2" && Convert.ToString(Switch.GetParameter(300)) == "2"))
				{
					...
					else
					{
						...
						do
						{
							...
							blockStaticText.Text = String.Format(message, trigger[2], swTo);
